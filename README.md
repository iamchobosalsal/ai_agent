...
최신 배포: [v3.6.95](downloads/v3.6.95) · [v3.7.43](downloads/v3.7.43). **두 버전 모두** Hermes 이미지 작업의 LM Studio 호환성 수정과, 이전 실패 대화에 남은 이미지 형식을 안전하게 전달하는 보완을 포함합니다. 기존 `/rag`, 모델 설정과 추론·출력 예산은 유지합니다. [변경 내역](CHANGELOG.md)
...
3. 처음 설치한다면 `v3.7.43`, 기존 대시보드와 상세 관리 기능을 사용하려면 `v3.6.95` 폴더를 엽니다.
...
| 버전 | 권장 대상 | 특징 |
|---|---|---|
| **v3.7.43** | 처음 설치하는 사용자 | 초보자용 UI, GPU 교체 자동 감지, 16GB GPU용 Gemma 4 선택지, Discord 연동, 자연어 RAG 근거 보충 |
| **v3.6.95** | 기존 화면과 상세 관리 기능을 선호하는 사용자 | 대시보드 중심 UI, Cloud API, Qwen3 Embedding 기반 RAG, Discord 연동, Hermes 스킬의 로컬 실행 도구 자동 설치·복구 |

처음 사용하는 경우 **v3.7**을 권장합니다.

### v3.6.95 · v3.7.43 이미지 작업 호환성 수정

이미지 입력을 지원하는 모델도 도구 메시지의 이미지 배열은 거부할 수 있습니다. 이 차이를 구분하고, 잘못된 메시지 형식 오류를 컨텍스트 초과로 오인해 대화를 압축·재시도하던 경로를 수정했습니다. 기존 실패 기록이 있는 대화도 삭제하지 않고 전송 형식만 호환 처리합니다. 중복 이미지는 앞선 사용자 원본을 유지하며, 도구에만 있는 이미지는 기존 보조 이미지 분석 경로로 처리합니다. 마지막 요약 요청도 같은 처리를 사용합니다.

실제 실패 대화 재생, 별도 폴더의 영수증 XLSX·연월별 이미지 복사, 회귀 검사 202개와 서명·내장 런타임 무결성을 검증했습니다. 모델의 코드 작성 오류나 흐린 사진의 OCR 오류까지 없어진 것은 아닙니다. 이전 반복 입력 캐시·Windows 작업 도구 개선은 유지합니다. [v3.6 시험 조건과 한계](RELEASE_NOTES_v3.6.95.md) · [v3.7 적용·검증 범위](RELEASE_NOTES_v3.7.43.md)를 확인하세요.

### 기존 사용자 업데이트 — 반드시 AI도 다시 시작하세요

1. 기존 런처에서 **AI 끄기**를 누르고 종료 완료를 기다린 뒤 런처를 닫습니다.
2. 새 배포 폴더를 완전히 압축 해제하고 `1_Install_...bat`을 관리자 권한으로 실행합니다.
3. 새 런처를 열어 **v3.6.95 또는 v3.7.43**인지 확인한 뒤 **AI 켜기**를 누릅니다.
4. 모델 서버와 Hermes·메신저 연결 시작 완료를 확인합니다.

**BAT 설치 또는 런처 창 재실행만으로는 이미 실행 중인 Hermes에 수정분이 반영되지 않을 수 있습니다.** 이미 설치했다면 런처에서 **AI 끄기 → AI 켜기**를 실행하세요. PC 재부팅이나 초기화·제거·모델 재다운로드는 필요하지 않습니다. 기존 모델·문서·대화·예약·연결 설정을 지우지 마세요.
...
3. `downloads/v3.7.43` 또는 `downloads/v3.6.95` 폴더를 엽니다.
...
v3.6.95·v3.7.43은 검색된 문서의 원본 XLSX 행과 인접 문맥을 제한적으로 보충합니다. 구체적인 모델명뿐 아니라 조건형 구성·추천 질문에도 관련 후보를 제공하며, 검색되지 않았다는 이유만으로 재고 0이나 해당 항목의 부재를 단정하지 않도록 보완했습니다. 문서의 예시·템플릿과 실제 업무 규칙도 구분하도록 안내합니다.
...
이번 버전은 메신저의 기본 경로에서 최종 답변을 삭제·대체하던 후처리와 도구 선택 개입을 해제했습니다. 일반 대화와 최신 조회를 구분하는 짧은 지침과 요청 시각을 제공하며, 도구 목록·모델 설정·최종 답변은 강제로 바꾸지 않습니다. 별도 검수 모델·검색 호출은 추가하지 않았습니다.

답변 삭제 문제의 수정이 사실 검증 완료를 의미하지는 않습니다. 검색 자료의 날짜·대상·수치를 잘못 연결하거나 서로 다른 업체 설명을 혼동하는 오류는 남아 있습니다. 중요한 정보는 출처의 대상·기준 시각·단위를 확인하세요. 모든 답변의 정확성이나 속도를 보장하는 배포는 아닙니다.
