...
최신 배포: [v3.6.90](downloads/v3.6.90) · [v3.7.38](downloads/v3.7.38). 자연어 `/rag`의 원본 표·인접 문맥 보충과 예시·업무 규칙 구분을 개선했습니다. [변경 내역](CHANGELOG.md)

`downloads`에는 **3.6·3.7 계열별 최신 배포판 하나씩만** 유지합니다. 이전 배포판은 현재 목록에서 제거하며 Git 변경 이력에서 확인할 수 있습니다.
...
3. 처음 설치한다면 `v3.7.38`, 기존 대시보드와 상세 관리 기능을 사용하려면 `v3.6.90` 폴더를 엽니다.
...
| 버전 | 권장 대상 | 특징 |
|---|---|---|
| **v3.7.38** | 처음 설치하는 사용자 | 초보자용 UI, GPU 교체 자동 감지, 16GB GPU용 Gemma 4 선택지, Discord 연동, 자연어 RAG 근거 보충 |
| **v3.6.90** | 기존 화면과 상세 관리 기능을 선호하는 사용자 | 대시보드 중심 UI, Cloud API, Qwen3 Embedding 기반 RAG, Discord 연동, Hermes 스킬의 로컬 실행 도구 자동 설치·복구 |
...
3. `downloads/v3.7.38` 또는 `downloads/v3.6.90` 폴더를 엽니다.
...
v3.7은 마지막 실행 때의 GPU 이름·VRAM·권장 등급과 현재 하드웨어를 비교합니다. GPU가 바뀌거나 VRAM 차이가 크면 예전 PC에서 저장된 모델을 계속 사용하지 않고 현재 GPU에 맞는 모델을 다시 선택할 수 있게 안내합니다.
...
| Gemma 4 12B | 12GB 이상 | 빠른 응답과 여유 있는 메모리 사용 |
| Gemma 4 26B A4B IT QAT | 16GB 이상 | 16GB급 GPU의 품질 우선 선택 |
...
등록한 문서는 사용자의 PC 안에서 청크와 임베딩으로 처리됩니다. v3.6은 Qwen3-Embedding-0.6B를 사용하며, 질문을 특정 업무 유형으로 고정하지 않고 자연어 그대로 검색한 뒤 관련 문서와 작업 규칙을 적용해 답변합니다. 일반 대화와 문서 질문을 구분하려면 질문 앞에 `/rag`를 붙일 수 있습니다.

v3.6.90·v3.7.38은 검색된 문서의 원본 XLSX 행과 인접 문맥을 제한적으로 보충합니다. 구체적인 모델명뿐 아니라 조건형 구성·추천 질문에도 관련 후보를 제공하며, 검색되지 않았다는 이유만으로 재고 0이나 해당 항목의 부재를 단정하지 않도록 보완했습니다. 문서의 예시·템플릿과 실제 업무 규칙도 구분하도록 안내합니다.

보충은 등록된 자료와 정해진 범위 안에서 수행하며 추가 모델 호출이나 재임베딩을 요구하지 않습니다. 원본 읽기와 근거 길이에 따른 처리 시간은 추가될 수 있고, 모든 조건·호환성이나 답변의 무오류를 보장하지는 않습니다. 중요한 수치와 조건은 인용된 원본을 확인하세요.
...
로컬 AI를 선택하면 언어모델 추론과 등록 문서 처리는 기본적으로 사용자의 PC에서 수행됩니다. Cloud API를 선택하면 질문과 RAG가 검색한 근거 일부가 선택한 외부 공급자로 전송될 수 있습니다. 원본 문서와 임베딩 DB는 PC에 남습니다. 다음 기능은 인터넷 연결 또는 외부 서비스 통신이 필요합니다.
