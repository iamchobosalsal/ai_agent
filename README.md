...
최신 배포: [v3.6.94](downloads/v3.6.94) · [v3.7.42](downloads/v3.7.42). **두 버전 모두** 지원 모델의 반복 입력 캐시와 Windows 로그·ZIP 사진 정리 도구 연결 개선을 포함합니다. 이번 **v3.7.42**는 v3.6.94의 공통 개선을 초보자용 런처에도 적용한 버전입니다. 기존 `/rag` 경로와 추론·출력 예산은 유지합니다. [변경 내역](CHANGELOG.md)
...
3. 처음 설치한다면 `v3.7.42`, 기존 대시보드와 상세 관리 기능을 사용하려면 `v3.6.94` 폴더를 엽니다.
...
| 버전 | 권장 대상 | 특징 |
|---|---|---|
| **v3.7.42** | 처음 설치하는 사용자 | 초보자용 UI, GPU 교체 자동 감지, 16GB GPU용 Gemma 4 선택지, Discord 연동, 자연어 RAG 근거 보충 |
| **v3.6.94** | 기존 화면과 상세 관리 기능을 선호하는 사용자 | 대시보드 중심 UI, Cloud API, Qwen3 Embedding 기반 RAG, Discord 연동, Hermes 스킬의 로컬 실행 도구 자동 설치·복구 |

처음 사용하는 경우 **v3.7**을 권장합니다.

### v3.6.94 · v3.7.42 작업 지연 개선

지원 모델의 컨텍스트 체크포인트를 활성화해 반복 입력의 중간 상태를 재사용하고, Hermes가 설치된 Windows 로그 분석기·ZIP 사진 정리기를 사용할 수 있도록 경로와 호출 방법을 제공합니다. 로그 수집 범위와 한글 출력, 한글 경로·동명 사진·원본 ZIP 보존을 검증했습니다.

첫 질문이나 대용량 파일 작업이 항상 빨라지는 것은 아니며 체크포인트에는 추가 메모리 비용이 있습니다. 로그 조회만으로 PC 전체가 정상이라고 단정할 수 없고, 모든 진단 추론의 정확성을 보장하지 않습니다. [v3.6 시험 조건과 한계](RELEASE_NOTES_v3.6.94.md) · [v3.7 적용·검증 범위](RELEASE_NOTES_v3.7.42.md)를 확인하세요.
...
3. 새 런처를 열어 **v3.6.94 또는 v3.7.42**인지 확인한 뒤 **AI 켜기**를 누릅니다.
...
3. `downloads/v3.7.42` 또는 `downloads/v3.6.94` 폴더를 엽니다.
...
v3.6.94·v3.7.42은 검색된 문서의 원본 XLSX 행과 인접 문맥을 제한적으로 보충합니다. 구체적인 모델명뿐 아니라 조건형 구성·추천 질문에도 관련 후보를 제공하며, 검색되지 않았다는 이유만으로 재고 0이나 해당 항목의 부재를 단정하지 않도록 보완했습니다. 문서의 예시·템플릿과 실제 업무 규칙도 구분하도록 안내합니다.
