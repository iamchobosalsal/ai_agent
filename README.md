...
최신 배포: [v3.6.92](downloads/v3.6.92) · [v3.7.40](downloads/v3.7.40). 메신저에서 방문 예정일 때문에 추천 답변 전체가 차단되던 문제를 수정하고 중복 안내문을 줄였습니다. 기존 자연어 `/rag` 개선도 포함합니다. [변경 내역](CHANGELOG.md)
...
3. 처음 설치한다면 `v3.7.40`, 기존 대시보드와 상세 관리 기능을 사용하려면 `v3.6.92` 폴더를 엽니다.
...
| 버전 | 권장 대상 | 특징 |
|---|---|---|
| **v3.7.40** | 처음 설치하는 사용자 | 초보자용 UI, GPU 교체 자동 감지, 16GB GPU용 Gemma 4 선택지, Discord 연동, 자연어 RAG 근거 보충 |
| **v3.6.92** | 기존 화면과 상세 관리 기능을 선호하는 사용자 | 대시보드 중심 UI, Cloud API, Qwen3 Embedding 기반 RAG, Discord 연동, Hermes 스킬의 로컬 실행 도구 자동 설치·복구 |
...
3. `downloads/v3.7.40` 또는 `downloads/v3.6.92` 폴더를 엽니다.
...
v3.6.92·v3.7.40은 검색된 문서의 원본 XLSX 행과 인접 문맥을 제한적으로 보충합니다. 구체적인 모델명뿐 아니라 조건형 구성·추천 질문에도 관련 후보를 제공하며, 검색되지 않았다는 이유만으로 재고 0이나 해당 항목의 부재를 단정하지 않도록 보완했습니다. 문서의 예시·템플릿과 실제 업무 규칙도 구분하도록 안내합니다.
...
이번 버전은 방문·구매 예정일과 자료의 유효 날짜를 구분하고, 검색 결과에 실제로 있는 설명도 출처와 함께 활용하도록 보완했습니다. 잘못된 수치가 있는 독립 항목·필드를 제한하면서 근거가 있는 다른 설명까지 통째로 지우지 않도록 수정했습니다. 중복된 안내문을 줄였으며 별도 검수 모델·검색 호출은 추가하지 않았습니다.

이 검사는 모든 문장의 진위를 판별하는 기능이 아닙니다. 모델의 상호·메뉴명 혼동이나 출처에 없는 속성 추가 등 의미상 오류는 남을 수 있으므로 중요한 정보는 원문을 확인하세요.
