...
최신 배포: [v3.6.93](downloads/v3.6.93) · [v3.7.41](downloads/v3.7.41). 메신저 답변을 삭제·대체하던 기본 후처리를 해제하고, 한글 MCP 응답·스킬 본문 로딩·설정 갱신을 수정했습니다. 기존 `/rag` 경로와 모델 설정은 유지합니다. [변경 내역](CHANGELOG.md)
...
3. 처음 설치한다면 `v3.7.41`, 기존 대시보드와 상세 관리 기능을 사용하려면 `v3.6.93` 폴더를 엽니다.
...
| **v3.6.93** | 기존 화면과 상세 관리 기능을 선호하는 사용자 | 대시보드 중심 UI, Cloud API, Qwen3 Embedding 기반 RAG, Discord 연동, Hermes 스킬의 로컬 실행 도구 자동 설치·복구 |
...
3. 새 런처를 열어 **v3.6.93 또는 v3.7.41**인지 확인한 뒤 **AI 켜기**를 누릅니다.
...
3. `downloads/v3.7.41` 또는 `downloads/v3.6.93` 폴더를 엽니다.
...
v3.6.93·v3.7.41은 검색된 문서의 원본 XLSX 행과 인접 문맥을 제한적으로 보충합니다. 구체적인 모델명뿐 아니라 조건형 구성·추천 질문에도 관련 후보를 제공하며, 검색되지 않았다는 이유만으로 재고 0이나 해당 항목의 부재를 단정하지 않도록 보완했습니다. 문서의 예시·템플릿과 실제 업무 규칙도 구분하도록 안내합니다.
