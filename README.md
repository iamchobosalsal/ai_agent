...
최신 배포: [v3.6.94](downloads/v3.6.94) · [v3.7.41](downloads/v3.7.41). **v3.6.94**는 지원 모델의 반복 입력 캐시와 Windows 로그·ZIP 사진 정리 도구 연결을 개선했습니다. 기존 `/rag` 경로와 추론·출력 예산은 유지하며, **v3.7.41은 이번에 변경하지 않았습니다.** [변경 내역](CHANGELOG.md)
...
3. 처음 설치한다면 `v3.7.41`, 기존 대시보드와 상세 관리 기능을 사용하려면 `v3.6.94` 폴더를 엽니다.
...
| 버전 | 권장 대상 | 특징 |
|---|---|---|
| **v3.7.41** | 처음 설치하는 사용자 | 초보자용 UI, GPU 교체 자동 감지, 16GB GPU용 Gemma 4 선택지, Discord 연동, 자연어 RAG 근거 보충 |
...
첫 질문이나 대용량 파일 작업이 항상 빨라지는 것은 아니며 체크포인트에는 추가 메모리 비용이 있습니다. 로그 조회만으로 PC 전체가 정상이라고 단정할 수 없고, 모든 진단 추론의 정확성을 보장하지 않습니다. [시험 조건과 한계](RELEASE_NOTES_v3.6.94.md)를 확인하세요.
...
3. 새 런처를 열어 **v3.6.94 또는 v3.7.41**인지 확인한 뒤 **AI 켜기**를 누릅니다.
...
3. `downloads/v3.7.41` 또는 `downloads/v3.6.94` 폴더를 엽니다.
...
v3.6.94·v3.7.41은 검색된 문서의 원본 XLSX 행과 인접 문맥을 제한적으로 보충합니다. 구체적인 모델명뿐 아니라 조건형 구성·추천 질문에도 관련 후보를 제공하며, 검색되지 않았다는 이유만으로 재고 0이나 해당 항목의 부재를 단정하지 않도록 보완했습니다. 문서의 예시·템플릿과 실제 업무 규칙도 구분하도록 안내합니다.
