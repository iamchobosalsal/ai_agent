AI One-touch v3.6.93 - 무료 내부 서명 MSIX 배포 도구
==================================================

v3.6.93 업데이트
- 메신저 기본 경로에서 답변을 삭제·대체하던 후처리와 도구 선택 개입을 해제했습니다.
- MCP 한글 UTF-8 응답과 출처 필드/줄바꿈, 알려진 k-skill 스텁의 실제 본문 로딩, 안전한 설정 갱신을 수정했습니다.
- 모델·LM Studio·검색 서비스 기본값과 기존 /rag 경로는 유지합니다. 추가 검수 모델·검색·재임베딩을 강제하지 않습니다.
- 검색 자료의 날짜·대상·수치를 잘못 연결하는 오류는 남아 있습니다. 중요한 내용은 원문을 확인하세요. 모든 답변의 정확성/속도를 보장하지 않습니다.
- 모델·문서·연결 설정·예약·대화는 초기화하지 않습니다. 별도 Team AI는 포함하지 않습니다.

기존 사용자 업데이트 — AI 끄기 → 설치 → AI 켜기
1. 기존 런처에서 AI 끄기 → 종료 완료 확인 → 런처 닫기.
2. 새 배포 폴더 압축 해제 → 1_Install_AI_OneTouch_Internal.bat을 관리자 권한으로 실행.
3. 새 런처의 v3.6.93 확인 → AI 켜기 → 모델 서버와 Hermes·메신저 연결 시작 완료 확인.
주의: BAT 설치나 창 재실행만으로는 실행 중인 Hermes가 갱신되지 않을 수 있습니다.
이미 설치했다면 AI 끄기 → AI 켜기를 실행하세요. PC 재부팅·초기화·제거·모델 재다운로드는 필요하지 않습니다.
기존 모델·문서·대화·예약·연결 설정을 지우지 마세요. 상세 내용은 RELEASE_NOTES_v3.6.93.md를 참고하세요.

이전 v3.6.90 업데이트
- /rag가 등록 XLSX의 원본 행과 검색된 문서의 인접 문맥을 제한적으로 보충합니다.
- 구체적인 모델명 없이 조건형 구성·추천 질문을 해도 문서의 분류별 후보를 찾도록 보완했습니다.
- 검색 누락과 실제 부재·재고 0, 문서의 예시와 상시 업무 규칙을 구분하도록 안내합니다.
- 추가 모델 호출·재임베딩은 없지만 원본 읽기와 추가 문맥 처리 비용은 있습니다. 모든 답변의 무오류를 보장하지는 않습니다.
- 기존 모델·문서·연결 설정을 유지합니다. 업데이트 후 런처를 다시 열고 AI 끄기 → AI 켜기를 한 번 실행하세요.
- 별도 Team AI 서버·클라이언트는 포함하지 않습니다.

이전 v3.6.88 업데이트
- Telegram·Discord·Slack 고정 예약 알림은 실행 시 모델을 거치지 않고 전달합니다.
- 예약 보고가 무응답을 반환하면 성공으로 숨기지 않고 실패를 기록·통지합니다.
- 예약 인증, 반복·취소, 전송 오류 기록 및 날짜·근거 검증을 보완했습니다.
- 기존 대화·모델·연결 설정은 유지합니다. 설치 후 AI 끄기 → AI 켜기로 적용하세요.
- PC가 꺼져 있거나 절전·오프라인이면 제시간 전달을 보장할 수 없습니다.
...
- 설정을 바꾼 뒤에는 AI를 껐다가 다시 켜야 Hermes Desktop과 Telegram/Discord/Slack에 적용됩니다.

Discord 빠른 연결
1. Discord 왼쪽 + 버튼에서 봇을 사용할 서버를 만듭니다.
2. 런처의 'Discord Bot 설정 열기'에서 Application과 Bot을 만듭니다.
3. Bot 화면에서 발급한 Bot Token을 런처에 입력합니다. Application ID나 Client ID를 넣는 곳이 아닙니다.
4. Bot > Privileged Gateway Intents에서 Message Content Intent를 켭니다.
5. Discord 사용자 설정 > 고급 > 개발자 모드를 켜고 내 프로필에서 '사용자 ID 복사'를 누릅니다.
6. 복사한 숫자를 런처의 '허용 사용자 ID'에 입력합니다. 역할·채널·알림 채널 ID는 처음에는 비워도 됩니다.
...
9. 런처에서 '설정 저장' > '저장하고 연결'을 누르고 대시보드에서 AI를 켭니다.
...
- 봇이 오프라인: 런처에서 저장하고 연결한 뒤 대시보드에서 AI를 켭니다.
