AI One-touch Beginner v3.7.24 - 무료 내부 서명 MSIX 배포 도구
==================================================
...
v3.7.24 변경 사항
- GPU 교체 후에도 이전 GPU용 모델이 계속 선택되던 문제를 수정했습니다.
- 현재 GPU 이름과 VRAM을 저장된 하드웨어 정보와 비교해 권장 모델을 다시 선택합니다.
- 16GB급 GPU에는 Gemma 4 26B A4B IT, 32GB급 GPU에는 Qwen 3.6 35B-A3B를 권장합니다.
- GPU가 바뀌지 않았다면 사용자가 직접 저장한 모델 선택은 유지합니다.
- 이전에 내려받은 모델 파일은 자동으로 삭제하지 않습니다.
