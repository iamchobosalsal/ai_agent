AI One-touch v3.6.95 - 무료 내부 서명 MSIX 배포 도구
==================================================
...
- 원본 백업·중복 적용·알 수 없는 Hermes 구조에서의 비수정 처리를 검증했습니다. 추가 답변 삭제 규칙이나 재고/날씨별 예외 처리는 없습니다.
- 흐린 사진의 글자·세금·부호를 잘못 읽을 가능성은 남아 있습니다. 중요한 전표는 원본과 대조하세요. 모든 OCR 정확도를 보장하지 않습니다.
- 모델·RAG·대화·예약·Team AI는 변경하지 않습니다.

이전 v3.6.94 업데이트
- LM Studio 컨텍스트 체크포인트를 활성화해 지원 모델의 반복 입력 재처리를 줄였습니다. 첫 입력이나 모든 작업의 속도를 보장하는 것은 아니며 메모리 사용량이 추가될 수 있습니다.
- Hermes에 Windows 로그 분석기와 ZIP 사진·영상 정리기의 실제 경로와 사용법을 제공합니다. 해당 스크립트 호출에만 실행 정책 Bypass를 지정하며 전역 정책은 변경하지 않습니다.
- 로그 수집의 시간·수량 제한, 한글 출력, 오류/빈 결과 구분과 재부팅 기록 보존을 검증했습니다. 원본 ZIP·메모·동명 사진 보존 및 표현을 바꾼 자연어 요청도 검증했습니다.
- PC 전체의 정상 상태나 모든 답변의 정확도를 보장하지 않습니다. 모델·대화 길이·출력 한도·RAG 검색 엔진은 변경하지 않습니다. 3.7과 Team AI는 이번 수정 대상이 아닙니다.
...
3. 새 런처의 v3.6.95 확인 → AI 켜기 → 모델 서버와 Hermes·메신저 연결 시작 완료 확인.
주의: BAT 설치나 창 재실행만으로는 실행 중인 Hermes가 갱신되지 않을 수 있습니다.
이미 설치했다면 AI 끄기 → AI 켜기를 실행하세요. PC 재부팅·초기화·제거·모델 재다운로드는 필요하지 않습니다.
기존 모델·문서·대화·예약·연결 설정을 지우지 마세요. 상세 내용은 RELEASE_NOTES_v3.6.95.md를 참고하세요.
