...
set "MSIX=%BASE%AI_OneTouch_v3.6_Internal.msix"
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%SHORTCUT_TOOL%" -PackageName "AI.OneTouch.Internal" -ApplicationId "AIOneTouch" -ShortcutName "AI One-touch Internal"
if errorlevel 1 (
  echo WARNING: The app was installed, but the desktop shortcut could not be created.
  echo You can still open AI One-touch Internal from the Start menu.
) else (
  echo Desktop shortcut created: AI One-touch Internal
)

echo.
echo Installation completed. Open AI One-touch Internal from the desktop or Start menu.
pause
exit /b 0
...
echo 3. For a clean recovery, run Reset_Tool.exe and then run this installer again.
pause
exit /b 1
