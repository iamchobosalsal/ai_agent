AI One-touch Beginner v3.7.43 - 무료 내부 서명 MSIX 배포 도구
==================================================
...
이전 v3.7.42 업데이트
- v3.6.94에서 검증한 반복 입력 캐시·Windows 작업 도구 연결 개선을 초보자용 v3.7에도 적용했습니다.
- 지원 모델의 컨텍스트 체크포인트를 4개로 설정합니다. 모델 종류·대화 길이·추론/출력 예산·RAG는 줄이거나 변경하지 않습니다.
- Windows 로그 분석기·ZIP 사진 정리기의 실제 경로와 호출 방법을 안내합니다. 보조 스크립트 실행에만 -ExecutionPolicy Bypass를 사용하며 시스템 전체 정책은 바꾸지 않습니다.
- 로그 조회의 시간·수량 제한, 한글 출력, 권한 오류와 정상 빈 결과 구분 및 반환된 재부팅 기록 보존을 보완했습니다.
- 캐시는 모델·입력 재사용 여부에 따라 효과가 다르며 추가 메모리를 사용합니다. 최초 입력·대용량 파일 작업의 속도나 모든 진단 추론의 정확성을 보장하지 않습니다.
- v3.7 전용 UI·모델 선택과 기존 기능을 유지합니다. 별도 Team AI는 포함하지 않습니다.
...
3. 새 런처의 v3.7.43 확인 → AI 켜기 → 모델 서버와 Hermes·메신저 연결 시작 완료 확인.
주의: BAT 설치나 창 재실행만으로는 실행 중인 Hermes가 갱신되지 않을 수 있습니다.
이미 설치했다면 AI 끄기 → AI 켜기를 실행하세요. PC 재부팅·초기화·제거·모델 재다운로드는 필요하지 않습니다.
기존 모델·문서·대화·예약·연결 설정을 지우지 마세요. 상세 내용은 RELEASE_NOTES_v3.7.43.md를 참고하세요.
