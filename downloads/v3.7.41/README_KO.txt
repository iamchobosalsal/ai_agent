...
2. 새 배포 폴더 압축 해제 → 1_Install_AI_OneTouch_Beginner_Internal.bat을 관리자 권한으로 실행.
3. 새 런처의 v3.7.41 확인 → AI 켜기 → 모델 서버와 Hermes·메신저 연결 시작 완료 확인.
주의: BAT 설치나 창 재실행만으로는 실행 중인 Hermes가 갱신되지 않을 수 있습니다.
이미 설치했다면 AI 끄기 → AI 켜기를 실행하세요. PC 재부팅·초기화·제거·모델 재다운로드는 필요하지 않습니다.
기존 모델·문서·대화·예약·연결 설정을 지우지 마세요. 상세 내용은 RELEASE_NOTES_v3.7.41.md를 참고하세요.
...
이전 v3.7.36 업데이트
- Telegram·Discord·Slack 공통 예약 모듈의 고정 알림은 실행 시 모델을 거치지 않고 전달합니다.
